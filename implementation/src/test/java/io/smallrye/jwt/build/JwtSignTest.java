...
public class JwtSignTest {
    ...
    public void testSignClaims() throws Exception {
        ...
        checkDefaultClaimsAndHeaders(jws, claims);
    ...
    private static void checkDefaultClaimsAndHeaders(JsonWebSignature jws, JwtClaims claims) throws Exception {
        checkDefaultClaimsAndHeaders(jws, claims, "RS256");
    }

    private static void checkDefaultClaimsAndHeaders(JsonWebSignature jws, JwtClaims claims, String algo) throws Exception {
        Assert.assertNotNull(claims.getIssuedAt());
        Assert.assertNotNull(claims.getExpirationTime());
        Assert.assertNotNull(claims.getJwtId());
        Assert.assertEquals(algo, jws.getAlgorithmHeaderValue());
        Assert.assertEquals("JWT", jws.getHeader("typ"));
    }
    ...
    public void testSignClaimsEllipticCurve() throws Exception {
        ...
        Assert.assertEquals(4, claims.getClaimsMap().size());
        checkDefaultClaimsAndHeaders(jws, claims, "ES256");
    ...
    public void testSignClaimsSymmetricKey() throws Exception {
        ...
        Assert.assertEquals(4, claims.getClaimsMap().size());
        checkDefaultClaimsAndHeaders(jws, claims, "HS256");
